...
class Transaction(models.Model):
    status_transaction = ((1, 'Charge'), (2, 'Purchase'),
                          (3, 'Transfer_sent'), (4, 'Transfer_received'))
...
class Transfer(models.Model):
    sender = models.ForeignKey(
        User, related_name="sent", on_delete=models.PROTECT)
    ...
    def transfer_amount(cls, email_sender, email_receiver, amount):
        ...
        with transaction.atomic():
            Transaction.objects.create(
                user=user_sender, amount=amount, status=3)
            Transaction.objects.create(
                user=user_reciver, amount=amount, status=4)
            cls.objects.create(sender=user_sender,
                               receiver=user_reciver, amount=amount)
